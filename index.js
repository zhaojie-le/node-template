...
program
  ...
  .action((name) => {
    ...
    if (fs.existsSync(endPath)) {
      console.log(symbols.error, chalk.red(`${file}文件夹已存在`));
      return
    }
      inquirer
        .prompt([
        ...
        .then(answers => {
          ...
          downLoadFile(type, endPath)
          .then(
            files => {
              spinner.succeed();
              if (type === 'react-component') {
                templateParse(files, endPath, type)
              }
